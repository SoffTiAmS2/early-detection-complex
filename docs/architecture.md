...
MVP intentionally dry-run: он не запускает контейнеры honeypot, а строит план модулей, пишет локальное applied state и отправляет `sensor.status`. Следующий практичный шаг - заменить dry-run для `cowrie` на module runner, переиспользовав проверенную сборку из `archive/prototype-v0/`.
