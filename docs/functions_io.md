...
Назначение: проверить, что API-manager запущен.
...
Назначение: отдать справочник honeypot, их сервисов и настроек.
...
Перед запуском нового контейнера playbook останавливает старый compose stack выбранного сенсора, удаляет orphan-контейнеры и leftover-контейнеры с compose label этого EDC-сенсора. Чужие Docker-контейнеры на плате глобально не удаляются.
...
Назначение: показать состояние collector и сенсоров через manager API.
